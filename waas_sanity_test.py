...
from sanity_util import COLLECTION, IVERSON, JOULE, WIN_INT_PORT, WIN_EXT_PORT, HOST_PORT, \
    ...
    CONTAINER_PORT_MOCK, CONTAINER_PORT_DVWA, KEPLER, OOB, AWS_COL, AWS_PORT
...
def get_auth():
    if customer_id is None:
        return ('1admin2', 'Pa$$word1NotForUseOnlyInCaseYoudon$thaveanotheroption')
    else:
...
def get_console_version():
    ...
    if response.status_code == 200:
        ...
        if version_number > KEPLER_VERSION:
...
def create_waas_custom_rules():
    ...
    response_custom_rule = json.loads('{"_id":1,"type":"waas-response","name":"request-custom-rule-for-sanity","message":"%resp.status_code is denied","script":"resp.status_code = 200"}')
...
def create_rule():
    ...
    rule["rules"][0]["applicationsSpec"][0]["customRules"] = [{"_id": offset, "effect":"disable"}, {"_id": offset + 1, "effect": "disable"}]
